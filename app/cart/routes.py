...
@cart_bp.route('/add_to_cart/<int:product_id>/<int:quantity>',
               methods=['POST'])
@login_required
def add_to_cart(product_id, quantity):
  ...
  if cart_item:
    # If item is already in the cart, update quantity and custom description
    cart_item.quantity += quantity
    cart_item.custom_description = custom_description
  else:
    # Otherwise, add a new item to the cart with custom description
...
def get_product_quantity(product_id):
    ...
    if cart_item:
        ...
        # Product not found, return quantity as 0
...
def update_cart():
  try:
    ...
    if user_id is None:
      raise ValueError(
          'User not authenticated')  # You can customize this error message
...
def checkout():
  ...
  if form.validate_on_submit():
    ...
    flash('Order placed successfully! Redirecting to payment page...',
          'success')
